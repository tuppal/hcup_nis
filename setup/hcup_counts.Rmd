...
#                ifelse(PAY1==3, "Private", ifelse(PAY1==4, #"Self-Pay", "Other"))))),
              ...
              RACE_co=ifelse(RACE==1, "White, Non-Hispanic",
                             ifelse(RACE==2, "Black, Non-Hispanic", ifelse(RACE==3, "Hispanic, any race", "Other"))))
...
nis_ip_obj<-dir(nis_diab_path)
results_path<-paste0(repo, "/tables/nis_tables/adult_diab")

x<-readRDS(file=paste0(nis_diab_path, "/", nis_ip_obj[3]))
y<-count_table_nis(x)
yr<-mean(x$variables$YEAR)
setwd(results_path)
cat(y, file=paste0("count_", yr,".html"))
...
for(i in(1:4)){
x<-readRDS(file=paste0(nis_diab_path, "/", nis_ip_obj[i]))
y<-count_table_nis(x)
yr<-mean(x$variables$YEAR)
setwd(results_path)
cat(y, file=paste0("count_",yr,".html"))
